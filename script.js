...
const fundraiserNotification = {
    enabled: true,
    title: "Final Hike: Ben Nevis",
    message: "On 15 August, I’ll take on the final hike of the fundraiser: Ben Nevis. Thank you for supporting Issac Lee and MAF UK all the way to the finish.",
    target: "2026-08-15T11:00",
    dateLabel: "Saturday 15 August 2026 · 11:00 am",
    buttonLabel: "View final hike",
    scrollTarget: "ben-nevis"
};
...
// =========================================================
const videos = [
    { key: "intro", title: "Challenge Intro", url: "https://www.youtube.com/shorts/nyoPz9B5pNw" },
    { key: "issac", title: "Issac & MAF UK", url: "https://www.youtube.com/watch?v=MApBRFexRIA" },
    { key: "snowdon", title: "Snowdon", url: "https://youtube.com/shorts/6Gty9ftO-uo" },
    { key: "scafell", title: "Scafell Pike", url: "https://www.youtube.com/shorts/jDoSZVH7xQU" },
    { key: "bennevis", title: "Ben Nevis", url: "" }
];
...
function initialiseFundraisingProgress() {
    ...
    if (tracker) tracker.setAttribute("aria-valuenow", String(currentRaised));

    const updateMarkerPosition = (percent) => {
        if (!progressMarker) return;
        progressMarker.style.bottom = `${percent}%`;
        progressMarker.style.transform = percent > 72 ? "translateY(110%)" : "translateY(50%)";
    };
...
function initialiseVideoPopup() {
    ...
    const renderVideo = () => {
        ...
        if (videoModalTitle) videoModalTitle.textContent = currentVideo.title;
        if (videoModalPrev) videoModalPrev.disabled = currentVideoIndex === 0;
        if (videoModalNext) videoModalNext.disabled = currentVideoIndex === validVideos.length - 1;
    };
    ...
    if (openVideoGallery) {
        if (validVideos.length) openVideoGallery.addEventListener("click", () => openVideoModal(0));
        else {
    ...
    if (videoModalPrev) videoModalPrev.addEventListener("click", () => {
    ...
    if (videoModalNext) videoModalNext.addEventListener("click", () => {
    ...
    if (closeVideoModal) closeVideoModal.addEventListener("click", closeVideoPopup);
    if (videoModalBackdrop) videoModalBackdrop.addEventListener("click", closeVideoPopup);

    document.querySelectorAll(".peak-watch-btn, .issac-video-btn").forEach((button) => {
        const matchingIndex = validVideos.findIndex((video) => video.key === button.dataset.hike);
        if (matchingIndex === -1) {
    ...
    document.addEventListener("keydown", (event) => {
        if (event.key === "Escape" && videoModal && videoModal.classList.contains("is-open")) closeVideoPopup();
    });
}
...
function initialiseNotificationPopup() {
    ...
    const updateCountdown = () => {
        ...
        const millisecondsRemaining = target.getTime() - Date.now();
    ...
    if (notificationTitle) notificationTitle.textContent = fundraiserNotification.title;
    if (notificationMessage) notificationMessage.textContent = fundraiserNotification.message;
    if (notificationDate) notificationDate.textContent = fundraiserNotification.dateLabel;

    if (notificationScrollButton) {
        notificationScrollButton.textContent = fundraiserNotification.buttonLabel;
        notificationScrollButton.href = `#${fundraiserNotification.scrollTarget}`;
    ...
    if (closeNotificationButton) closeNotificationButton.addEventListener("click", closeNotificationPopup);
    if (notificationModalBackdrop) notificationModalBackdrop.addEventListener("click", closeNotificationPopup);

    updateCountdown();
    window.setInterval(updateCountdown, 30000);

    document.addEventListener("keydown", (event) => {
        if (event.key === "Escape" && notificationModal && notificationModal.classList.contains("is-open")) closeNotificationPopup();
    });
}
...
function animateValue(element, start, end, duration, formatValue) {
    ...
    function update(currentTime) {
        ...
        element.textContent = formatValue(Math.round(start + (end - start) * easedProgress));
...
function toEmbedUrl(url, autoplay = false) {
    ...
    const videoId = (shortsMatch && shortsMatch[1]) || (watchMatch && watchMatch[1]) || (embedMatch && embedMatch[1]) || (shortLinkMatch && shortLinkMatch[1]) || cleanUrl;
